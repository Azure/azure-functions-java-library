...
Maven packages and plugins are restored through the `upstream-public` Azure Artifacts feed. The
root `pom.xml` overrides Maven's `central` repository, and `settings.xml` mirrors early plugin and
